...
class App
{
    ...
    public static function run(Request $request = null)
    {
        ...
        try {
            ...
            // 记录路由和请求信息
            if (self::$debug) {
                Log::record('[ ROUTE ] ' . json_encode($dispatch,JSON_UNESCAPED_UNICODE), 'info');
                Log::record('[ HEADER ] ' . json_encode($request->header(),JSON_UNESCAPED_UNICODE), 'info');
                Log::record('[ PARAM ] ' . json_encode($request->param(), JSON_UNESCAPED_UNICODE), 'info');
            }
    ...
    public static function initCommon()
    {
        if (empty(self::$init)) {
            ...
            $debugOperator=  Cache::get('debugOperator');
            $param=Request::instance()->param();
            if(!empty($debugOperator)&&
                (in_array('*',$debugOperator)||(!empty($param['operator'])&&in_array($param['operator'],$debugOperator)))){
                self::$debug =true;
            }
            if (!self::$debug) {
